...
\begin{equation}
    \begin{aligned}
    u_4 = [-0.2245,\ 0.4713,\ -0.8529]^\top \qquad u_7 = [-0.7100,\ 0.0828,\ -0.6993]^\top \\
    u_5 = [-0.0089,\ 0.7024,\ -0.7117]^\top \qquad u_8 = [-0.0124,\ 0.1176,\ -0.9930]^\top \\
    u_6 = [-0.4545,\ 0.1047,\ -0.8846]^\top \qquad u_9 = [0.2215,\ -0.2937,\ -0.9299]^\top \\
    \end{aligned}
\end{equation}
...
All six ground tensions lie close to $\tau_{\min}=5.5$ N. This result is expected: since the desired wrench is almost entirely vertical, the UAVs provide most of the lifting force while the Stewart cables primarily stabilize the payload orientation and prevent lateral motion. As a result, the optimizer drives the ground tensions toward their minimum admissible values, maintaining taut cables while minimizing unnecessary internal forces.
