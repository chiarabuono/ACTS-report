...
Using Equation~\ref{eq:cableVector}b, the ground cable directions $u_i$ and the corresponding Jacobian are obtained as:
...
The UAV positions are not predefined but result from the tension-planning procedure described in Section \ref{sec:tensionPlanner}. The corresponding cable directions $u_1, u_2, u_3$ are therefore extracted from a converged simulation run for this configuration rather than computed from known attachment coordinates:
\begin{equation}
    \begin{aligned}
a_1 = [1.1020, -0.2692, 3.5972]^\top \\
a_2 = [0.1802, -0.7553, 3.4976]^\top \\
a_3 = [0.9397, -1.2760, 3.5192]^\top
    \end{aligned}
\end{equation}
Using Equation~\ref{eq:cableVector}b, the resulting drone cable directions are
\begin{equation}
    \begin{aligned}
u_1 = [0.0837, -0.0294, 0.9961]^\top \\
u_2 = [0.1042, -0.3528, 0.9299]^\top \\
...
\begin{equation}
\begin{aligned}
\tau_{\text{UAV}} &= [13.6970,\ 11.1548,\ 14.4154]^\top~\text{N} \\
...
Eq.~\ref{eq:drone-disposition} reduces to $F_{prop,i} = \tau_i u_i + m_i g\,e_3$, giving the hand-computed thrust magnitudes
\begin{equation}
T_1 = 33.29~\text{N}, \qquad T_2 = 30.27~\text{N}, \qquad T_3 = 33.57~\text{N}.
\end{equation}
The controller-computed thrust commands for the same converged pose are $33.2793$, $30.2683$, and $33.5657$~N respectively, matching the hand-computed values to within numerical precision and confirming the correct implementation of Eq.\ref{eq:drone-disposition}.
...
A detailed description of the adopted statistical methods is provided in Appendix \ref{app:statistics}.
