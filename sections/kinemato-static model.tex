...
In the first tier, candidate ground-cable routings are screened using the ground-cable subsystem alone. For each routing, the ground-only payload Jacobian ($J_{p,\mathrm{ground}}$) is constructed from the six ground cables. The routing is then evaluated according to three criteria: the Wrench Closure Condition (WCC),

\begin{equation}
\operatorname{rank}(J_{p,\mathrm{ground}})=6,
\end{equation}

the Wrench Feasible Condition (WFC), formulated as a bounded tension feasibility problem and solved using linear programming, and the Interference-Free Condition (IFC), restricted to the ground-cable geometry. These tests are performed over a representative set of payload poses. A routing is discarded if it violates any of these criteria at any tested pose.

The WCC applied at this stage should be interpreted as a conservative pre-screening criterion rather than as a requirement on the final ACTS. Since the ground subsystem is considered independently, it must be capable of generating a full six-dimensional wrench by itself. The UAV cables are not yet included in this first-tier analysis.

The remaining routings are then ranked according to their worst-case performance over the representative workspace. In particular, the conditioning index and wrench-generation metrics introduced in Section \ref{sec:performanceIndices} can be used to identify routings that maintain a favorable geometric configuration throughout the workspace.

In the second tier, the complete ACTS architecture is evaluated for the most promising ground routings and UAV attachment geometries. The full payload Jacobian ($J_{p,\mathrm{full}}$) is assembled from the ground and aerial cables. The UAV positions are determined from the selected architecture and, where applicable, from the actuator-position optimization described in Section 4.2. The complete system is then evaluated using the combined cable-tension limits, including the maximum admissible tension imposed by the UAV thrust capabilities.
