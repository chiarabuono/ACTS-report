...
\end{itemize}
\todo[inline]{If the flight is quasi-stationary, do we really have a dynamic model or just a static model?}
...
\todo[inline]{Isn't it the opposite? Not small but big}
...
for the best routing found by conditioning index, the joint (ground+drone) Wrench Feasible Condition is re-evaluated using the nominal drone positions which place each drone directly above its payload hook. If this strict check fails, rather than discarding the routing outright, the drone positions are instead optimized -- within their fixed cable length -- to minimize the joint WFC residual (\texttt{optimize\_drone\_positions\_for\_wfc}), and the routing is accepted only if this repositioned configuration satisfies the WFC tolerance at every tested pose. An architecture is reported as \emph{feasible} if at least one routing survives WCC and IFC at every pose and passes the WFC check, either directly or after this rescue.
