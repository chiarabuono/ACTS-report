...
\begin{equation}
    W_p = J_p^\top \tau
\end{equation}
Let $W_p^*$ be the desired payload wrench, the control allocation problem to solve is
\begin{equation}
    J_p^\top \tau = W_p^*, \qquad \text{s.t}. \tau_i > 0
\end{equation}

\subsection{Static model}
Let $\tau=[\tau_{uav}^\top, \tau_{ugv}^\top]^\top\in \mathbb{R}^{m\times 1}$ be the cable tension vector, then
\begin{equation} 
\begin{aligned}
