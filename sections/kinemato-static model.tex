...
In the first tier, candidate ground-cable routings are screened using the ground-cable subsystem alone. For each routing, the ground-only payload Jacobian ($J_{p,\mathrm{ground}}$) is constructed from the six ground cables and evaluated against the Wrench Closure Condition (WCC),

\begin{equation}
\operatorname{rank}(J_{p,\mathrm{ground}})=6,
\label{eq:wcc}
\end{equation}

(implemented in practice as a singular-value threshold, $\sigma_{\min} > \varepsilon_{\mathrm{rank}}$, rather than an exact rank test) and the Interference-Free Condition (IFC), restricted to ground-cable pairs. A routing is discarded immediately if it violates either criterion at any tested pose.

Within this same pass, a nominal-position joint Wrench Feasible Condition is also evaluated: the UAV cables are included using a cheap, non-optimized placement directly above each payload hook, and the resulting nine-cable Jacobian is checked via the bounded least-squares residual of Eq.~\ref{eq:jointWFC}. Under the default configuration, this WFC outcome does not by itself eliminate a routing; it is recorded alongside the WCC/IFC result and used to rank the surviving candidates. A dedicated rescue stage is triggered only if no routing passes all three criteria cleanly: the top candidates by manipulability are then re-evaluated using an actuator-repositioning search (Nelder--Mead over each UAV's angular position on its cable sphere, minimizing the joint WFC residual), promoting the first candidate whose repositioned configuration satisfies the residual tolerance. This differs from the SLSQP-based drone-disposition optimizer used at runtime, since the two solve distinct objectives under distinct constraint sets and are not interchangeable.

In the second tier, the complete ACTS architecture is evaluated for the most promising ground routings and UAV attachment geometries using the full payload Jacobian ($J_{p,\mathrm{full}}$), assembled from the ground and aerial cables. The wrench feasibility of the complete system is verified by solving the bounded optimization problem

\begin{equation}
\min_{\boldsymbol{\tau}} \left\| J_{p,\mathrm{full}}^\top \boldsymbol{\tau} - \mathbf{w}^{*} \right\|_2
\label{eq:jointWFC}
\end{equation}
...
\label{eq:jointWFCbounds}
\end{equation}

where the bounds include the maximum admissible tension imposed by the UAV thrust capabilities. A configuration is considered feasible when the resulting wrench residual falls below a prescribed tolerance.

The wrench feasibility of the complete system is verified by solving the bounded optimization problem
...
A configuration is considered feasible when the resulting wrench residual is below a prescribed tolerance. The surviving configurations are subsequently compared using the performance metrics defined in Section \ref{sec:performanceIndices}, including the capacity margin, force and moment radius of available wrench, and actuator margin.

This two-tier strategy separates the inexpensive combinatorial screening of ground-cable routings from the more computationally demanding analysis of the complete aerial-ground system. It therefore reduces the number of configurations requiring full nonlinear evaluation while retaining the relevant UAV-dependent constraints for the final architecture selection.