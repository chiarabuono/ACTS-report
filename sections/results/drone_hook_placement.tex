...
Before evaluating the different UAV attachment topologies, each candidate payload-ground layout was first passed through the same ground-only pre-screening procedure described in Section \ref{sec:ground_screening} and Algorithm \ref{alg:cable_optimization}. Since this stage operates exclusively on the ground-cable Jacobian $J_{p,\text{ground}}$, it is, by construction, not strongly dependent of the UAV attachment geometry.
