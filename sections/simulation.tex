...
The only variables that change from one experiment to another are the cable routing, the UAV attachment geometry, and the placement of the ground hook points. At the start of the simulation, the payload is on the ground along with the drones, which are resting on its top surface.
...
In the next chapter we will answer questions like "Which configuration gives the best payload control performance according to the theoretical indices?" and "Which configuration would I actually choose for the robot?"
