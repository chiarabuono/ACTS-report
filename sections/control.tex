...
\todo[inline]{It is right - write it better}
...
Since the controller performs set point regulation,
...
The ground subsystem is modeled as a 6-DOF Gough-Stewart parallel mechanism where the six ground cables act as active legs. Given the target payload position $\mathbf{p}^*$ and target orientation matrix $\mathbf{R}^*$, the ideal length $\rho_i^*$ for the $i$-th ground cable is defined geometrically by:
\begin{equation}
    \rho_i^* = \| \mathbf{a}_i - (\mathbf{p}^* + \mathbf{R}^* \,^p \mathbf{b}_i) \|
\end{equation}
where $\mathbf{a}_i$ represents the fixed ground anchor (pulley exit point) and $^p\mathbf{b}_i$ is the hook location in the payload body frame.
...
To account for the physical velocity limits of the winch motors, the target length is tracked using a velocity-saturated kinematic controller. At each control step $\Delta t$, the commanded cable length extension is constrained by the maximum winch speed $v_{\text{max}}$:
\begin{equation}
    \rho_i(t + \Delta t) = \rho_i(t) + \text{sat}_{\Delta \rho_{\text{max}}} \left( \rho_i^* - \rho_i(t) \right)
\end{equation}
where the saturation bounds are given by $\Delta \rho_{\text{max}} = v_{\text{max}} \cdot \Delta t$, defined as:
\begin{equation}
    \text{sat}_{\Delta \rho_{\text{max}}}(e) = \max \left(-\Delta \rho_{\text{max}}, \, \min \left(e, \, \Delta \rho_{\text{max}}\right) \right)
\end{equation}

This commanded position reference $\rho_i(t + \Delta t)$ is directly fed to the actuator position servo. This ensures smooth, bounded rate transitions toward the target setpoint without demanding unrealistic acceleration or spooling speeds from the ground winches.
