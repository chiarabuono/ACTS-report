...
Then, the desired payload wrench $W_p^*$ is computed for each point in the trajectory based on the previous state of the system. One strategy is using a PD controller:
    \begin{equation}
        \begin{aligned}
            F^*_p &= m_p (\ddot{p}^* + g) + K_p (p^* - p) + K_d (\dot{p}^* - \dot{p}) \\
            M^*_p &= K_r e_r + K_w e_w
        \end{aligned}
    \end{equation}

\todo[inline]{How we computed er and ew and add paper reference}
